...
for a, b in ROUTES:
    ...
    ax.plot([x1,x2],[y1,y2], color=BLUE, lw=2.0, alpha=.75, zorder=1,
            solid_capstyle="round")
# label placement chosen per city so nothing collides
OFFSET = {"DEL":(0, 16, "center"), "BOM":(-12, 2, "right"), "BLR":(-12, -6, "right"),
          "CCU":(14, 0, "left"),   "HYD":(14, 4, "left"),   "MAA":(14, -10, "left")}
for code,(x,y,name) in CITY.items():
    ...
    ax.annotate(f"{code}\n{name}", (x,y), textcoords="offset points",
                ...
                color=INK, zorder=4)
ax.set_xlim(67, 94); ax.set_ylim(9, 32)
ax.axis("off")
ax.set_title("The fixed basket — six city pairs (schematic)",
             fontsize=13, fontweight="bold", color=NAVY, loc="left", pad=6)
ax.text(0.0, -0.01, "Trunk routes, a north-east leg and two short hauls.",
